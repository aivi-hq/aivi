...
if (endpoint.auth) {
  ...
  assert.equal((await fetch(`${endpoint.url}/api/health`, { headers: { authorization: `Bearer ${endpoint.auth.password}` } })).status, 401);
...
const created = await client.session.create({ id, agent: 'build', location: { directory }, title: 'aivi live check', metadata: { aivi: { origin: 'live-check' } } });
...
try {
  if (!values['skip-prompt']) {
    ...
    await client.session.prompt({ sessionID: id, id: `msg_aivi_${randomUUID().replaceAll('-', '')}`, text: 'Reply with exactly the word: pong', delivery: 'queue', model: { providerID, modelID } });
    ...
    report('prompt/wait/context', `${Date.now() - started}ms via ${providerID}/${modelID}: ${JSON.stringify(answer.content.filter(p => p.type === 'text').map(p => p.text).join(''))}`);
  }
