...
try {
  ...
  const run = (...args) => JSON.parse(execFileSync(process.execPath, [cli, '--config', config, ...args], { encoding: 'utf8' }));
  ...
  daemon = spawn(process.execPath, [cli, '--config', config, 'serve'], {
    env: { ...process.env, AIVI_TOKEN: token }, stdio: ['ignore', 'pipe', 'pipe'],
  });
  ...
  const listening = await new Promise((resolve, reject) => {
    ...
    daemon.once('exit', code => { clearTimeout(timeout); reject(new Error(`Host exited during startup: ${code}`)); });
    daemon.stdout.on('data', chunk => {
      buffer += chunk;
      if (buffer.includes('\n')) {
        clearTimeout(timeout);
        try { resolve(JSON.parse(buffer.split('\n')[0]).listening); } catch (error) { reject(error); }
      }
    });
  });
  ...
  assert.equal((await fetch(`http://127.0.0.1:${listening.port}/v1/status`)).status, 401, 'token mode rejects anonymous callers');
  ...
  await writeFile(config, JSON.stringify({ version: 1, host: { port: 0, auth: { mode: 'none' } }, knowledge: [{ id: 'demo', path: '.' }] }));
  daemon = spawn(process.execPath, [cli, '--config', config, 'serve'], { env: { ...process.env, AIVI_TOKEN: '' }, stdio: ['ignore', 'pipe', 'pipe'] });
  stopped = once(daemon, 'exit');
  const open = await new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error('Host startup timed out')), 10000);
    let buffer = '';
    daemon.once('exit', code => { clearTimeout(timeout); reject(new Error(`Host exited during startup: ${code}`)); });
    daemon.stdout.on('data', chunk => { buffer += chunk; if (buffer.includes('\n')) { clearTimeout(timeout); resolve(JSON.parse(buffer.split('\n')[0]).listening); } });
  });
  assert.equal((await fetch(`http://127.0.0.1:${open.port}/v1/status`, { signal: AbortSignal.timeout(5000) })).status, 200);
  daemon.kill('SIGTERM');
