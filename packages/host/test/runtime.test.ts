...
test('opencode.prompt jobs run a full verified turn and succeed with the final answer', async t => {
  ...
  const job = store.enqueue(taskSchema.parse({ kind: 'opencode.prompt', agent: 'librarian', directory: '/team', prompt: 'Read the handbook' }), 'local-model', 'request');
  const received: { method: string; path: string; body: Record<string, any>; attached: string | null; auth: string | undefined }[] = [];
  let promptId = '';
  const server = createServer(async (req, res) => {
    let raw = ''; for await (const chunk of req) raw += chunk;
    const body = raw ? JSON.parse(raw) : {};
    received.push({ method: req.method!, path: req.url!, body, attached: store.get(job.id).sessionId, auth: req.headers.authorization });
    res.setHeader('content-type', 'application/json');
    const url = req.url!;
    if (url.endsWith('/wait')) { res.writeHead(204); res.end(); return; }
    if (url.endsWith('/permission') && req.method === 'GET') { res.end('{"data":[]}'); return; }
    if (url.endsWith('/prompt')) { promptId = body.id; res.end(JSON.stringify({ data: { id: body.id } })); return; }
    if (url.endsWith('/context')) {
      res.end(JSON.stringify({ data: [
        { type: 'user', id: promptId, text: 'q', time: { created: 1 } },
        { type: 'assistant', id: 'a', agent: 'librarian', finish: 'stop', time: { created: 2, completed: 3 }, content: [{ type: 'text', text: 'Handbook read.' }] },
        { type: 'idle', id: 'i', outcome: 'succeeded', time: { created: 4 } },
      ] })); return;
    }
    res.end(JSON.stringify({ data: { id: body.id ?? received[0]!.body.id, agent: 'librarian', location: { directory: '/team' } } }));
  });
  ...
  const scheduler = new Scheduler(store, config.scheduler, createExecutor({ path: '/aivi.json', config, projects: [], sources: [] }, { store, opencode }));
  scheduler.tick(); await scheduler.drain(); scheduler.tick(); await scheduler.drain();
...
test('an unreachable OpenCode blocks the job instead of failing the scheduler', async t => {
  ...
  const job = store.enqueue(taskSchema.parse({ kind: 'opencode.prompt', agent: 'librarian', directory: '/team', prompt: 'hi' }), 'local-model', 'unreachable');
  const config = configSchema.parse({ version: 1 });
  const opencode = async () => { throw new Error('No running OpenCode v2 service found'); };
  const scheduler = new Scheduler(store, config.scheduler, createExecutor({ path: '/aivi.json', config, projects: [], sources: [] }, { store, opencode }));
...
test('shell tasks run argv without a shell, capture output, and map exit codes to states', async t => {
  ...
  const opencode = async () => { throw new Error('not needed'); };
  const ok = store.enqueue(taskSchema.parse({ kind: 'shell', command: [process.execPath, '-e', 'console.log("hi $HOME"); console.error("warn")'] }), 'local-model', 'ok');
  const bad = store.enqueue(taskSchema.parse({ kind: 'shell', command: [process.execPath, '-e', 'process.exit(3)'] }), 'local-model', 'bad');
  const scheduler = new Scheduler(store, { ...config.scheduler, maxConcurrent: 2, resources: { 'local-model': 2 } }, createExecutor(loaded, { store, opencode }));
  scheduler.tick(); await scheduler.drain();
  const done = store.get(ok.id);
  assert.equal(done.state, 'succeeded');
  assert.deepEqual(done.result, { exitCode: 0, signal: null, stdout: 'hi $HOME\n', stderr: 'warn\n' }, 'no shell expansion');
...
test('a shell task that exceeds its timeout is blocked, not failed', async t => {
  const store = new Store(':memory:'); t.after(() => store.close());
  const config = configSchema.parse({ version: 1, stateDirectory: '/tmp' });
  const job = store.enqueue(taskSchema.parse({ kind: 'shell', command: [process.execPath, '-e', 'setTimeout(()=>{}, 10000)'], timeoutMs: 1000 }), 'local-model', 'slow');
  const scheduler = new Scheduler(store, config.scheduler, createExecutor({ path: '/aivi.json', config, projects: [], sources: [] }, { store, opencode: async () => { throw new Error('x'); } }));
  scheduler.tick(); await scheduler.drain();
