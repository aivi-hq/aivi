...
test('startup applies removed schedules before dispatching any stale queued occurrence', async t => {
  const store = new Store(':memory:'); t.after(() => store.close());
  store.syncSchedules([{ id: 'old', cron: '* * * * *', timezone: 'UTC', resource: 'local-model', enabled: true, task: { kind: 'system.check' } }], 0);
  store.materializeDue(60_000);
  let calls = 0;
  const scheduler = new Scheduler(store, config, async () => { calls++; return { state: 'succeeded', result: null }; });
