...
import { finalAnswer, runTurn, PermissionRequired } from '../src/session.ts';
...
test('only the matching completed final answer is returned, without reasoning or tool data', () => {
  const messages = [
    ...
    { type: 'user', id: 'current', time: { created: 2 }, text: 'current question', metadata: { aivi: { message: 'msg_one' } } },
    { type: 'assistant', id: 'answer', time: { created: 3, completed: 4 }, agent: 'librarian', finish: 'stop',
      content: [{ type: 'reasoning', text: 'private reasoning' }, { type: 'text', text: 'Public answer' }] },
  ...
  assert.equal(finalAnswer(messages, 'current', 'librarian'), 'Public answer', 'the native message id also identifies the turn');
...
function mockOpenCode(options: { pending?: { id: string; action: string; resources: string[] }[]; agent?: string } = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    const url = req.url!;
    if (url.endsWith('/permission/rules') || url.endsWith('/wait')) { res.writeHead(204); res.end(); return; }
    if (url.endsWith('/permission') && req.method === 'GET') { res.end(JSON.stringify({ data: pending })); return; }
    if (/\/permission\/[^/]+\/reply$/.test(url)) { pending = pending.filter(p => !url.includes(p.id)); res.writeHead(204); res.end(); return; }
    ...
    if (url.endsWith('/context')) {
      res.end(JSON.stringify({ data: [
        ...
        { type: 'assistant', id: 'a', agent, finish: 'stop', time: { created: 2, completed: 3 }, content: [{ type: 'text', text: 'Answer' }] },
...
async function withServer<T>(t: { after(fn: () => Promise<void>): void }, mock: ReturnType<typeof mockOpenCode>, run: (client: Awaited<ReturnType<typeof connectOpenCode>>) => Promise<T>): Promise<T> {
...
test('runTurn creates, pins permissions, prompts, and returns the verified answer', async t => {
  ...
  let created = 0;
  const result = await withServer(t, mock, client => runTurn(client, { ...input, create: true, permissions: [{ action: '*', resource: '*', effect: 'deny' }], sessionMetadata: { aivi: { origin: 'test' } } }, { signal: AbortSignal.timeout(5000), pollMs: 10, onCreated: () => created++ }));
  assert.deepEqual(result, { sessionId: 'ses_test', text: 'Answer', rejected: [] });
  assert.equal(created, 1);
  const order = mock.requests.map(r => `${r.method} ${r.path.replace(/^\/api\/session\/ses_test/, '')}`.replace('/api/session', 'create'));
...
test('runTurn auto-rejects permission prompts by default and reports them', async t => {
  ...
  const result = await withServer(t, mock, client => runTurn(client, { ...input, create: false }, { signal: AbortSignal.timeout(5000), pollMs: 10 }));
...
test('runTurn with onPermission fail leaves the prompt pending and throws PermissionRequired', async t => {
  ...
  await withServer(t, mock, client => assert.rejects(
    runTurn(client, { ...input, create: false }, { signal: AbortSignal.timeout(5000), pollMs: 10, onPermission: 'fail' }),
...
test('runTurn refuses a session whose agent changed', async t => {
  ...
  await withServer(t, mock, client => assert.rejects(
    runTurn(client, { ...input, agent: 'developer', create: false }, { signal: AbortSignal.timeout(5000), pollMs: 10 }),
