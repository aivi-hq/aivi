...
import type { LoadedConfig, KnowledgeService, Logger } from '@aivi/core';
...
export function createExecutor(loaded: LoadedConfig, deps: ExecutorDeps): Execute {
  ...
  return async (job, context) => {
    switch (job.task.kind) {
      ...
      case 'system.check': {
        const sources = await Promise.all(loaded.sources.map(async source => {
          const available = await access(source.path).then(() => true, () => false);
      ...
      case 'shell': {
        ...
        const outcome = await new Promise<{ exitCode: number | null; signal: string | null; stdout: string; stderr: string }>(resolve => {
          execFile(file!, args, { cwd, timeout: task.timeoutMs, maxBuffer: 1024 * 1024, signal: context.signal, windowsHide: true }, (error, stdout, stderr) => {
            const exec = error as (NodeJS.ErrnoException & { code?: number | string; signal?: string; killed?: boolean }) | null;
            resolve({
              exitCode: typeof exec?.code === 'number' ? exec.code : exec ? null : 0,
              signal: exec?.signal ?? null,
              stdout: tail(stdout), stderr: exec && typeof exec.code !== 'number' ? tail(`${stderr}\n${exec.message}`) : tail(stderr),
            });
          });
        });
        ...
        if (outcome.exitCode === null) return { state: 'blocked', result: outcome, reason: outcome.signal ? `Command killed by ${outcome.signal}` : 'Command did not exit cleanly' };
      ...
      case 'dreaming': {
        ...
        try {
          const outcome = await dream(job.task, job.id, {
            store: deps.store, opencode: deps.opencode, stateDirectory: loaded.config.stateDirectory,
            signal: AbortSignal.any([context.signal, AbortSignal.timeout(job.task.timeoutMs)]), log,
          });
          if (outcome.result.reviewed && deps.knowledge) await deps.knowledge.index().catch(error => log.warn('dreaming.index.failed', { error }));
          return outcome;
        } catch (error) {
          // The cursor did not advance, so the next run reviews the same conversations again.
          log.warn('dreaming.failed', { error });
          const reason = context.signal.aborted ? 'Host stopped during dreaming'
            : error instanceof Error && error.name === 'TimeoutError' ? `Dreaming exceeded ${job.task.timeoutMs}ms`
            : errorMessage(error);
          return { state: 'blocked', result: { sessionId }, reason: `${reason}. Inspect session ${sessionId} and resolve this job.` };
        }
      }
      case 'opencode.prompt': {
        ...
        try {
          const turn = await runTurn(client, {
            sessionId, agent: job.task.agent, directory: job.task.directory, create: true,
            title: `aivi ${job.id}`, sessionMetadata: metadata,
            messageId: `msg_aivi_${suffix}`, text: job.task.prompt, messageMetadata: metadata,
          }, {
            signal: AbortSignal.any([context.signal, AbortSignal.timeout(job.task.timeoutMs)]),
            onPermission: job.task.onPermission, log,
          });
          ...
          const reason = error instanceof PermissionRequired ? error.message
            : context.signal.aborted ? 'Host stopped while the turn was running'
            : error instanceof Error && error.name === 'TimeoutError' ? `Turn exceeded ${job.task.timeoutMs}ms`
            : errorMessage(error);
          return { state: 'blocked', result: { sessionId }, reason: `${reason}. Inspect session ${sessionId} and resolve this job.` };
        }
      }
