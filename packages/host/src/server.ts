...
import type { LoadedConfig, Status, KnowledgeService, BrowserService, Logger, KnowledgeKind } from '@aivi/core';
...
export function resolveHostAuth(mode: 'none' | 'token', token: string | undefined): HostAuth {
  ...
  if (!token) throw new Error('AIVI_TOKEN is required while host.auth.mode is "token". Set it in the environment (fnox) or set host.auth.mode to "none" on a trusted network.');
  if (token.length < MIN_TOKEN_LENGTH) throw new Error(`AIVI_TOKEN must contain at least ${MIN_TOKEN_LENGTH} characters`);
...
export function createHostServer({ store, loaded, auth, knowledge, browser, log = silentLogger }: HostServerOptions) {
  ...
  return createServer((request, response) => {
    ...
    const send = (code: number, body: unknown) => { response.writeHead(code); response.end(JSON.stringify(body)); };
    ...
    if (!authorized(request)) { send(401, { error: 'Unauthorized' }); return; }

    if (url.pathname === '/v1/browser') { handleBrowser(request, response, send); return; }
    if (request.method !== 'GET') { send(405, { error: 'Method not allowed' }); return; }
    if (url.pathname === '/v1/status') { send(200, status(store, loaded)); return; }
    if (url.pathname === '/v1/knowledge/search') { handleSearch(url, send); return; }
    if (url.pathname === '/v1/sources') { handleSources(url, send); return; }
  ...
  function parseSelection(params: URLSearchParams, extra: string[]): { projects?: string[]; includeCore: boolean; kinds?: KnowledgeKind[] } | string {
  ...
  function handleSources(url: URL, send: Send) {
    ...
    if (typeof selection === 'string') { send(400, { error: selection }); return; }
    try { send(200, selectSources(loaded, selection.projects, selection.includeCore, selection.kinds)); }
  ...
  function handleSearch(url: URL, send: Send) {
    ...
    if (!parsed.success) { send(400, { error: 'Invalid search request' }); return; }
    try { selectSources(loaded, parsed.data.projects, parsed.data.includeCore, parsed.data.kinds); }
    catch (error) { send(400, { error: (error as Error).message }); return; }
    if (!knowledge) { send(503, { error: 'Knowledge search is unavailable' }); return; }
    const { query, limit, projects, includeCore, kinds } = parsed.data;
    knowledge.search({ query, limit, ...(projects ? { projects } : {}), ...(includeCore !== undefined ? { includeCore } : {}), ...(kinds ? { kinds } : {}) }).then(
      hits => send(200, hits),
      error => { log.warn('knowledge.search.failed', { error }); send(503, { error: 'Knowledge search is unavailable' }); },
    );
  }

  function handleBrowser(request: IncomingMessage, response: ServerResponse, send: Send) {
    if (request.method !== 'POST') { send(405, { error: 'Use POST for browser operations' }); return; }
    if (!browser) { send(503, { error: 'Browser is not configured' }); return; }
    if (!request.headers['content-type']?.startsWith('application/json')) { send(415, { error: 'Expected JSON' }); return; }
    void (async () => {
      const chunks: Buffer[] = [];
      let bytes = 0;
      for await (const chunk of request as AsyncIterable<Buffer>) {
        bytes += chunk.length;
        if (bytes > MAX_BROWSER_BODY) { send(413, { error: 'Browser request is too large' }); return; }
        chunks.push(chunk);
      }
      const parsed = browserEnvelopeSchema.safeParse(safeJson(Buffer.concat(chunks).toString('utf8')));
      if (!parsed.success) { send(400, { error: 'Invalid browser request' }); return; }
      try { send(200, await browser.execute(parsed.data.sessionId, parsed.data.request)); }
      catch (error) {
        log.warn('browser.failed', { session: parsed.data.sessionId, action: parsed.data.request.action, error });
        send(409, { error: 'Browser operation failed. Inspect the owned tabs before retrying; uncertain connections require a host restart.' });
      }
    })().catch(error => {
