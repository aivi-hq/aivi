...
export interface Lease { id: string; owner: string; resource: string; state: 'running' | 'blocked'; createdAt: number; reason: string | null }
export interface AuditEntry { seq: number; jobId: string; at: number; action: string; reason: string }
const lease = (r: Row): Lease => ({
  id: String(r.id), owner: String(r.owner), resource: String(r.resource), state: r.state as Lease['state'],
  createdAt: Number(r.created_at), reason: r.reason === null ? null : String(r.reason),
});
const audit = (r: Row): AuditEntry => ({ seq: Number(r.seq), jobId: String(r.job_id), at: Number(r.at), action: String(r.action), reason: String(r.reason) });
const states: JobState[] = ['queued', 'running', 'succeeded', 'failed', 'blocked', 'cancelled'];
const hash = (v: unknown) => createHash('sha256').update(JSON.stringify(v)).digest('hex');
const job = (r: Row): Job => ({
  id: String(r.id), task: JSON.parse(String(r.task)) as Task, resource: String(r.resource),
  state: r.state as JobState, createdAt: Number(r.created_at), scheduledFor: Number(r.scheduled_for),
  startedAt: r.started_at === null ? null : Number(r.started_at),
  finishedAt: r.finished_at === null ? null : Number(r.finished_at),
  scheduleId: r.schedule_id as string | null, sessionId: r.session_id as string | null,
  owner: r.owner as string | null, error: r.error as string | null,
...
export class Store {
  ...
  migrate(namespace: string, steps: readonly string[]): void {
    this.transaction(() => {
      ...
      this.db.prepare('INSERT INTO migrations(namespace,version) VALUES(?,?) ON CONFLICT(namespace) DO UPDATE SET version=excluded.version')
  ...
  note(id: string, action: string, reason: string, now = Date.now()): void { this.record(id, action, reason, now); }
  list(): Job[] { return (this.db.prepare('SELECT * FROM jobs ORDER BY created_at,id').all() as Row[]).map(job); }
  get(id: string): Job {
  ...
  counts(): Record<JobState, number> {
    ...
    for (const r of this.db.prepare('SELECT state,count(*) AS n FROM jobs GROUP BY state').all()) result[r.state as JobState] = Number(r.n);
    return result;
  }
  enqueue(task: Task, resource: string, dedupeKey: string, now = Date.now(), report: Report | null = null): Job {
    return this.transaction(() => this.insert(task, resource, dedupeKey, now, now, null, report));
  }
  private insert(task: Task, resource: string, key: string, now: number, due: number, scheduleId: string | null, report: Report | null): Job {
    ...
    const id = randomUUID();
    this.db.prepare(`INSERT INTO jobs(id,dedupe_key,fingerprint,task,resource,state,created_at,scheduled_for,schedule_id,report)
      VALUES(?,?,?,?,?,'queued',?,?,?,?)`).run(id, key, fingerprint, JSON.stringify(task), resource, now, due, scheduleId, report ? JSON.stringify(report) : null);
    this.record(id, 'enqueued', scheduleId ? `schedule:${scheduleId}` : 'operator', now);
    return this.get(id);
  }
  syncSchedules(schedules: Schedule[], now = Date.now()): void {
    this.transaction(() => {
      this.db.prepare('UPDATE schedules SET enabled=0').run();
      for (const spec of schedules) {
        const fingerprint = hash(spec);
        const current = this.db.prepare('SELECT fingerprint,next_at FROM schedules WHERE id=?').get(spec.id);
        if (current && current.fingerprint !== fingerprint) {
          for (const row of this.db.prepare("SELECT id FROM jobs WHERE state='queued' AND schedule_id=?").all(spec.id)) {
            this.db.prepare("UPDATE jobs SET state='cancelled',finished_at=? WHERE id=?").run(now, String(row.id));
            this.record(String(row.id), 'cancelled', 'schedule definition changed', now);
          }
        }
        const next = current?.fingerprint === fingerprint ? Number(current.next_at) : nextOccurrence(spec.cron, spec.timezone, now);
        this.db.prepare(`INSERT INTO schedules(id,spec,fingerprint,next_at,enabled) VALUES(?,?,?,?,?)
      ...
      for (const row of this.db.prepare(`SELECT id FROM jobs WHERE state='queued' AND schedule_id IS NOT NULL
        AND schedule_id IN (SELECT id FROM schedules WHERE enabled=0)`).all()) {
        this.db.prepare("UPDATE jobs SET state='cancelled',finished_at=? WHERE id=?").run(now, String(row.id));
  ...
  materializeDue(now = Date.now()): number {
    return this.transaction(() => {
      let created = 0;
      for (const row of this.db.prepare('SELECT * FROM schedules WHERE enabled=1 AND next_at<=? ORDER BY next_at,id').all(now)) {
        const spec = JSON.parse(String(row.spec)) as Schedule;
        const outstanding = this.db.prepare("SELECT id FROM jobs WHERE schedule_id=? AND state IN ('queued','running','blocked')").get(spec.id);
        if (!outstanding) {
          this.insert(spec.task, spec.resource, `schedule:${spec.id}:${row.next_at}`, now, Number(row.next_at), spec.id, spec.report ?? null);
          created++;
        }
        // Coalesce downtime to one occurrence and skip ticks while one remains outstanding.
        this.db.prepare('UPDATE schedules SET next_at=? WHERE id=?').run(nextOccurrence(spec.cron, spec.timezone, now), spec.id);
      }
      return created;
    });
  }
  claim(owner: string, maxConcurrent: number, resources: Record<string, number>, now = Date.now()): Job | null {
    return this.transaction(() => {
      ...
      for (const row of this.db.prepare("SELECT * FROM jobs WHERE state='queued' AND scheduled_for<=? ORDER BY scheduled_for,created_at,id").all(now)) {
        const resource = String(row.resource);
        const limit = resources[resource];
        if (limit === undefined || (counts.get(resource) ?? 0) >= limit) continue;
        this.db.prepare("UPDATE jobs SET state='running',owner=?,started_at=? WHERE id=?").run(owner, now, String(row.id));
   ...
   */
  acquireLease(id: string, owner: string, resource: string, maxConcurrent: number,
    resources: Record<string, number>, onAcquire: () => void = () => {}, now = Date.now()): boolean {
    return this.transaction(() => {
  ...
  blockLease(id: string, owner: string, reason: string): void {
    const result = this.db.prepare("UPDATE resource_leases SET state='blocked',reason=? WHERE id=? AND owner=?").run(reason, id, owner);
    if (Number(result.changes) !== 1) throw new Error('Lost resource lease ownership');
  }
  ...
  blockLeasesOwnedBy(owner: string, reason: string): number {
    return Number(this.db.prepare("UPDATE resource_leases SET state='blocked',reason=? WHERE owner=? AND state='running'").run(reason, owner).changes);
  }
  leases(): Lease[] { return (this.db.prepare('SELECT * FROM resource_leases ORDER BY created_at,id').all() as Row[]).map(lease); }
  leaseCount(): number { return Number(this.db.prepare('SELECT count(*) AS n FROM resource_leases').get()!.n); }
  attachSession(id: string, owner: string, sessionId: string): void {
    const result = this.db.prepare("UPDATE jobs SET session_id=? WHERE id=? AND owner=? AND state='running'").run(sessionId, id, owner);
    if (Number(result.changes) !== 1) throw new Error('Lost job ownership while attaching session');
  }
  finish(id: string, owner: string, state: 'succeeded' | 'failed' | 'blocked', result: unknown, reason: string, now = Date.now()): void {
    this.transaction(() => {
      const update = this.db.prepare("UPDATE jobs SET state=?,result=?,error=?,finished_at=? WHERE id=? AND owner=? AND state='running'")
        .run(state, JSON.stringify(result ?? null), state === 'succeeded' ? null : reason, state === 'blocked' ? null : now, id, owner);
  ...
  cancelQueued(id: string, now = Date.now()): void {
    this.transaction(() => {
      const update = this.db.prepare("UPDATE jobs SET state='cancelled',finished_at=? WHERE id=? AND state='queued'").run(now, id);
      if (Number(update.changes) !== 1) throw new Error('Only queued jobs can be cancelled; running work needs reconciliation');
  ...
  resolveBlocked(id: string, outcome: 'succeeded' | 'failed', reason: string, now = Date.now()): void {
    ...
    this.transaction(() => {
      const update = this.db.prepare("UPDATE jobs SET state=?,error=?,finished_at=? WHERE id=? AND state='blocked'")
  ...
  acquireDaemon(owner: string, pid = process.pid, now = Date.now()): void {
    this.transaction(() => {
      ...
      this.db.prepare('INSERT INTO daemon(id,owner,pid) VALUES(1,?,?) ON CONFLICT(id) DO UPDATE SET owner=excluded.owner,pid=excluded.pid').run(owner, pid);
      for (const row of this.db.prepare("SELECT id FROM jobs WHERE state='running'").all()) {
        this.db.prepare("UPDATE jobs SET state='blocked',error='Host interrupted; reconcile external work before releasing capacity' WHERE id=?").run(String(row.id));
        this.record(String(row.id), 'blocked', 'restart recovery', now);
      }
    });
  }
  releaseDaemon(owner: string): void { this.db.prepare('DELETE FROM daemon WHERE id=1 AND owner=?').run(owner); }
  history(id: string): AuditEntry[] { return (this.db.prepare('SELECT * FROM audit WHERE job_id=? ORDER BY seq').all(id) as Row[]).map(audit); }
}
