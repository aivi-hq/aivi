...
export function upgradeCli(): void {
  const list = spawnSync('npm', ['list', '-g', 'aivi', '--depth=0'], { encoding: 'utf8' });
  if (list.status !== 0 || !list.stdout.includes('aivi@'))
    throw new Error('This CLI was not installed with npm; update it the way you installed it.');
  const install = spawnSync('npm', ['install', '-g', 'aivi@latest'], { stdio: 'inherit' });
  if (install.status !== 0) throw new Error(`npm install -g aivi@latest failed (exit ${install.status ?? 'signal'})`);
}
