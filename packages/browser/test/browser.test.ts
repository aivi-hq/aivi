...
import { createBrowserService, chromeArguments } from '../src/index.ts';
...
function fixture() {
  let next = 1; let closed = 0;
  const all = [{ id: next++, url: 'https://human.invalid/private', title: 'Human tab', selected: true }];
  const calls: { name: string; args: Record<string, unknown> }[] = [];
  const transport: BrowserTransport = {
    async call(name, args) {
      calls.push({ name, args });
      if (name === 'new_page') { all.forEach(p => p.selected = false); all.push({ id: next++, url: String(args.url), title: 'Blank', selected: true }); }
      const page = all.find(p => p.id === args.pageId);
      if (name === 'navigate_page') { assert.ok(page); page.url = String(args.url); page.title = 'Owned page'; }
      if (name === 'close_page') { assert.ok(page); all.splice(all.indexOf(page), 1); }
      if (name === 'take_snapshot') return { structuredContent: { snapshot: { uid: '1_0', name: page!.title }, pages: structuredClone(all) } };
...
test('two sessions own separate opaque tabs; page lists and snapshots do not leak other tabs', async () => {
  const fake = fixture(); const service = createBrowserService(config(), fake.transport);
  ...
  assert.deepEqual(await service.execute('session-a', { action: 'snapshot', tabId: a.tabId }), { snapshot: { uid: '1_0', name: 'Owned page' } });
  ...
  await service.close(); assert.equal(fake.closed(), 1);
});

test('tab limits are atomic under simultaneous opens; shutdown drains and rejects new work', async () => {
  const fake = fixture(); const cfg = config(); cfg.maxTabsPerSession = 1;
...
test('navigation failure preserves ownership; uncertain transport failures block later calls without retry', async () => {
  ...
  fake.transport.call = async () => { throw new Error('connection lost'); };
...
test('reconnect notices block stale ownership rather than adopting new page IDs', async () => {
  const transport: BrowserTransport = { async call() { return { structuredContent: { reconnected: true } }; }, async close() {} };
...
test('bounded queue rejects overflow while preserving accepted operations', async () => {
  ...
  const cfg = config(); cfg.maxPending = 1;
  const service = createBrowserService(cfg, { call: () => new Promise(resolve => { release = resolve; }), async close() {} });
  const first = service.execute('s', { action: 'tabs' });
  await assert.rejects(service.execute('s', { action: 'tabs' }), /queue is full/);
  release({ structuredContent: { pages: [] } }); await first; await service.close();
});

test('configuration preserves profiles, limits attachment to loopback, and rejects arbitrary code/file operations', () => {
  assert.deepEqual(chromeArguments(config()), ['--no-usage-statistics','--no-performance-crux','--pageIdRouting=true','--userDataDir=/aivi/chrome','--autoConnect']);
  ...
  assert.throws(() => browserConfigSchema.parse({ connection: { mode: 'attach', browserUrl: 'http://outside.invalid:9222' } }));
  assert.throws(() => browserRequestSchema.parse({ action: 'open', url: 'file:///private' }));
  assert.throws(() => browserRequestSchema.parse({ action: 'open', url: 'https://a.invalid', sessionId: 'someone-else' }));
...
test('real pinned MCP starts over stdio and advertises explicit page routing without launching Chrome', async () => {
  ...
  const cfg = browserConfigSchema.parse({ connection: { mode: 'attach', browserUrl: 'http://127.0.0.1:1' }, timeoutMs: 10000 });
