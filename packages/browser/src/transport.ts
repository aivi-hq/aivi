...
export interface McpReply { isError?: boolean; structuredContent?: Record<string, unknown> }
...
export function chromeArguments(config: BrowserConfig): string[] {
  ...
  if (connection.mode === 'attach') args.push(`--browserUrl=${connection.browserUrl}`);
    ...
    if (connection.mode === 'existing') args.push('--autoConnect');
      ...
      args.push(`--headless=${connection.headless}`, '--ignoreDefaultChromeArg=--disable-extensions',
...
export function createChromeTransport(config: BrowserConfig): BrowserTransport {
  ...
  async function connect(): Promise<Client> {
    ...
    return connecting ??= (async () => {
      ...
      transport = new StdioClientTransport({ command: process.execPath, args: [entry, ...chromeArguments(config)],
        env: { ...getDefaultEnvironment(), ...desktopEnv, CHROME_DEVTOOLS_MCP_NO_USAGE_STATISTICS: '1', CHROME_DEVTOOLS_MCP_NO_UPDATE_CHECKS: '1' },
        stderr: 'pipe' });
  ...
  return {
    async call(name, args) {
      ...
      return await connection.callTool({ name, arguments: args }, undefined, { timeout: config.timeoutMs }) as McpReply;
    },
    async close() {
      closed = true;
      try { await connecting; } catch { /* Failed startup still owns its transport. */ }
      await client?.close();
