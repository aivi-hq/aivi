...
const browserInput = {
  ...
  properties: {
    action: { type: 'string', enum: ['tabs', 'open', 'navigate', 'snapshot', 'click', 'fill', 'press', 'dialog', 'focus', 'close'] },
    tabId: { type: 'string', description: 'Tab owned by this session (from open/tabs). Required for every action except tabs/open.' },
...
const selectionProperties = {
  projects: { type: 'array', items: { type: 'string' }, description: 'Project IDs. Omit for all projects; [] for core only. Project knowledge is only relevant to that project unless the question compares projects.' },
  includeCore: { type: 'boolean', description: 'Include company-wide core sources (default true).' },
  kinds: { type: 'array', items: { type: 'string', enum: knowledgeKindNames }, description: `Restrict to kinds of material. ${knowledgeKindHelp}` },
} as const;
...
export default Plugin.define({
  ...
  async setup(ctx) {
    ...
    const registration = await ctx.tool.transform(editor => {
      ...
      editor.add({
        name: 'search',
        description: 'Keyword search over configured knowledge with source paths and excerpts. Omit projects for all; [] selects core only; includeCore defaults true.',
        input: {
          type: 'object',
          required: ['query'],
          additionalProperties: false,
        ...
        execute: async input => json(await client.search(input as { query: string; projects?: string[]; includeCore?: boolean; limit?: number; kinds?: KnowledgeKind[] })),
      });
      editor.add({
        name: 'status',
      ...
      editor.add({
        ...
        options: { namespace: 'aivi', codemode: true },
        execute: async input => json(await client.sources(input as { projects?: string[]; includeCore?: boolean; kinds?: KnowledgeKind[] })),
      });
      editor.add({
        name: 'control',
        description: 'Control this session’s Chrome tabs. Start with open or tabs; snapshot returns element uids for click/fill. Refresh snapshot after navigation. The shared profile retains logins; use focus for manual login. Never put passwords or secrets in fill. No automatic retries after uncertain actions.',
        input: browserInput,
