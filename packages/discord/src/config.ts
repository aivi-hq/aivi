...
export const discordConfigSchema = z.strictObject({
  ...
  if (!config.messageContent && config.access.channels.some(c => c.trigger !== 'mention')) {
    ctx.addIssue({ code: 'custom', path: ['messageContent'], message: 'triggers other than "mention" need messageContent: true (Discord only delivers unmentioned message text with that intent)' });
  }
});
