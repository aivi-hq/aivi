...
test('config rejects ambiguous Linear app ownership and invalid schedule resources', () => {
  assert.equal(configSchema.safeParse({ version: 1, linear: { applications: {
    developer: { agent: 'dev' }, reviewer: { agent: 'dev' },
  } } }).success, false);
  assert.equal(configSchema.safeParse({ version: 1, schedules: [{
    id: 'check', cron: '* * * * *', resource: 'missing', task: { kind: 'system.check' },
  }] }).success, false);
  assert.equal(configSchema.safeParse({ version: 1, schedules: [{
    id: 'check', cron: '* * * * *', timezone: 'Mars/Olympus', task: { kind: 'system.check' },
  }] }).success, false);
...
test('project files own lane mappings and paths; source selection never falls back on an unknown project', async t => {
  ...
  await writeFile(join(root, 'aivi.json'), JSON.stringify({
    version: 1, knowledge: [{ id: 'company', path: 'handbook' }],
    projects: [{ id: 'website', directory: 'checkout' }],
    linear: { applications: { worker: { agent: 'dev' } } },
  }));
  await writeFile(join(root, 'checkout/aivi.project.json'), JSON.stringify({
  ...
  assert.deepEqual(selectSources(loaded, ['website']).map(s => s.path), [join(root, 'handbook'), join(root, 'checkout/docs/adr')]);
...
test('calendar calculations use the configured timezone across daylight saving changes', () => {
  const pattern = '0 9 * * *';
  assert.equal(new Date(nextOccurrence(pattern, 'Europe/Amsterdam', Date.parse('2026-03-28T00:00:00Z'))).toISOString(), '2026-03-28T08:00:00.000Z');
  assert.equal(new Date(nextOccurrence(pattern, 'Europe/Amsterdam', Date.parse('2026-03-29T00:00:00Z'))).toISOString(), '2026-03-29T07:00:00.000Z');
});
